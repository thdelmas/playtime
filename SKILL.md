---
name: playtime
description: "The developmental organ — deliberate, safe-to-fail, intrinsically-motivated engagement that builds capability before it's needed. Where the other organs maintain what exists, playtime grows the repertoire: rehearse an unfamiliar tool in a sandbox before depending on it, stress-test a skill on weird inputs to find its blind spots, try novel recombinations of capabilities, follow curiosity into a throwaway toy — then harvest the learning into memory. Engage and learn, with failure free. Complements rem-sleep (consolidate experience you HAD ↔ generate new experience on purpose) and consciousness-loop (what to do with safe idle windows instead of empty ticks). TRIGGERS: 'playtime', 'play mode', 'engage and learn', 'sandbox this', 'mess around with', 'kick the tires on', 'rehearse', 'practice', 'tinker with', 'learn by doing', 'noodle on', 'try a new tool before using it', 'what to do with idle time'."
---
...
This is the complement to [rem-sleep](https://github.com/thdelmas/rem-sleep): sleep *consolidates experience you had*; play *generates new experience on purpose*. And it's what [consciousness-loop](https://github.com/thdelmas/consciousness-loop) should do with a safe idle window instead of an empty tick — an agent with nothing pending should be *developing*, not just dormant.
...
A space where failure costs nothing: a scratch directory, a sandbox, a throwaway branch, a toy/fake dataset, a disposable container. **The first rule of play: no production blast radius.** If a mistake here could touch anything real, it isn't a playground — find or make one that's truly disposable. (Play stays inside the body; it never crosses a boundary — that's [immune-check](https://github.com/thdelmas/immune-check)'s line.)
...
Drop the toy. Delete the playground, or — if the play accidentally grew into something real — graduate it into a proper project (and one day [sunset](https://github.com/thdelmas/sunset) it). Then return to the loop, now measurably more capable than before you played.
